...
3. **查看波形** (需要GTKWave):
   ```bash
   make wave          # 基本波形查看
   make wave-config   # 使用预配置查看（推荐）
   make wave-info     # 查看波形文件信息
   ```
...
### 波形文件特性
- **文件**: `build/cpu_tb.vcd` (92KB)
- **信号数**: 251个关键信号
- **仿真时间**: 2.04ms (完整指令执行周期)
- **时钟频率**: 100MHz

### 关键信号组织
1. **基本控制信号**: 时钟、复位、PC
2. **指令流水线**: IF/ID/EX各阶段指令
3. **ALU和结果**: 计算过程和结果数据
4. **寄存器状态**: x1-x5寄存器实时变化
5. **数据转发**: 转发控制和转发数据
6. **分支跳转**: 分支预测和跳转控制

### 波形查看工具
- **GTKWave**: `make wave-config` (包含预配置信号)
- **在线查看**: 上传VCD到 https://wavedrom.com/
- **详细指南**: 参考 `waveform_guide.md`
