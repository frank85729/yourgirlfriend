...
module traffic_light_unequal_timing (
    input wire clk,           // 时钟信号
...
// 时间参数（假设时钟频率为1Hz）
parameter MAIN_GREEN_TIME = 27;    // 主道绿灯时间27秒
parameter BRANCH_GREEN_TIME = 17;  // 支道绿灯时间17秒
...
reg [5:0] timer;              // 计时器
reg [5:0] timer_next;
reg [5:0] green_time_limit;   // 当前状态的绿灯时间限制
...
always @(*) begin
    case (current_state)
        MAIN_GREEN_BRANCH_RED: green_time_limit = MAIN_GREEN_TIME;
        MAIN_RED_BRANCH_GREEN: green_time_limit = BRANCH_GREEN_TIME;
        default: green_time_limit = MAIN_GREEN_TIME;
    endcase
end
...
always @(*) begin
    ...
    case (current_state)
        MAIN_GREEN_BRANCH_RED: begin
            // 主道绿灯，支道红灯
            if (branch_car && timer >= green_time_limit) begin
                ...
                timer_next = 0;
            end else if (!branch_car && timer >= green_time_limit) begin
        ...
        MAIN_RED_BRANCH_GREEN: begin
            // 主道红灯，支道绿灯
            if (main_car && timer >= green_time_limit) begin
                ...
                timer_next = 0;
            end else if (!main_car && timer >= green_time_limit) begin
